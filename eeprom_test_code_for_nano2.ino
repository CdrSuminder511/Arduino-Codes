...
void f_write_byte(byte p_location, byte p_data)             // this function produces the write cycle of the i2c
{
  f_i2c_initialize();
  f_start_communication();
  f_send_device_address();
  f_send_memory_location(p_location);
  f_send_data(p_data);
  f_stop_command();
  delayMicroseconds(c_delay_6us);
}

byte f_read_byte(byte p_location)             // this function produces the write cycle of the i2c
{
  byte lv_byte;
  f_i2c_initialize();
  f_start_communication();
...
void loop()
{
  ...
  for (byte lv_address = 0xFF; lv_address > 0x00; lv_address-- )
  { byte lv_temp;
    f_write_byte(lv_address, lv_address);
    delay(20);

    lv_temp = f_read_byte(lv_address);
    delay(20);

    if (lv_temp == lv_address)              // if data does'nt match stuck in the loop
    {
